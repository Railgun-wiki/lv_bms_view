...
| widgets | obj, label, button, bar, chart, line | ~85 KB → ~35 KB |
...
瓶颈在于：
1. **blend_to_rgb565.c**（23.4KB .o）— 包含 5 种混合模式，仅 NORMAL 即可
2. **chart 控件**（41.6KB .o）— 可用自定义折线替代
3. **LVGL 核心**（obj+style+event+group+refr ≈ 143KB .o）— 不可避免

**可行的进一步精简方案：**

| 方案 | .o 节省 | -Os 后节省 | 难度 |
|------|---------|-----------|------|
| 移除 chart，自定义折线绘制 | 41.6 KB | 17-25 KB | 中 |
| 移除 bar，用 obj+宽度模拟 | 13.6 KB | 5-8 KB | 低 |
| 修改 blend_to_rgb565 移除非 NORMAL 模式 | ~10 KB | 4-6 KB | 中（改 LVGL 源码） |
| 禁用 A8 blend（接受无抗锯齿） | 8 KB | 3-5 KB | 低 |
| 禁用 LV_DRAW_SW_COMPLEX | ~15 KB | 6-10 KB | 低（但圆角/阴影失效） |
| **全部应用后** | **~88 KB** | **35-54 KB** |

全部应用后预估 Flash：**63-105 KB**。仍有可能超 64KB，但差距缩小到 0-41KB。
